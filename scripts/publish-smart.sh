...
SHOW_JSON="$(
  npx @vscode/vsce show "${EXT_ID}" --json "$@" 2>/dev/null || true
)"

if [[ -z "$SHOW_JSON" ]]; then
  echo "Could not read Marketplace version. Aborting to avoid an unintended publish." >&2
  echo "Run manually: npx @vscode/vsce publish ${CURRENT_VERSION}  OR  npx @vscode/vsce publish ${BUMP_KIND}" >&2
  exit 1
fi
...
if [[ "$CURRENT_VERSION" != "$PUBLISHED_VERSION" ]]; then
  ...
  exec npx @vscode/vsce publish "${CURRENT_VERSION}" "$@"
fi

echo "No version change detected. Publishing with automatic ${BUMP_KIND} bump..."
exec npx @vscode/vsce publish "${BUMP_KIND}" "$@"
